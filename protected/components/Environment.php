 ...
 * Yii::createWebApplication($env->webApplicationConfig)->run();
 * }}}
 *
 ...
 * Optional: in consoleApplicationConfig you can copy settings from webApplicationConfig by
 ...
 *     // Use value 'inherit' to copy from generated webApplicationConfig.
...
{
	/**
	 * Inherit key that can be used in consoleApplicationConfig
	 */
	 ...
	 */
	protected $modeFile = '../config/local.mode';
	 ...
	 */
	public $consoleApplicationConfig;
	...
	{
		// Load main config
		$fileMainConfig = $this->getConfigDir() . 'global.php';
		...
		$fileSpecificConfig = $this->getConfigDir().strtolower($this->mode).'.php';
	...
	protected function setEnvironment()
	{
		...
		$this->webApplicationConfig = $config['webApplicationConfig'];
		$this->webApplicationConfig['params']['environment'] = strtolower($this->mode);

		// Set console attributes and related actions
		if (isset($config['consoleApplicationConfig']) && !empty($config['consoleApplicationConfig'])) {
			$this->consoleApplicationConfig = $config['consoleApplicationConfig'];
			$this->processInherits($this->consoleApplicationConfig); // Process consoleApplicationConfig for inherits
			$this->consoleApplicationConfig['params']['environment'] = strtolower($this->mode);
		}
	...
	/**
	 * Loop through console config array, replacing values called 'inherit' by values from $this->webApplicationConfig
	...
	private function processInherits(&$array, $path = array())
	{
		foreach($array as $key => &$value) {
			...
			if ($value === self::INHERIT_KEY)
				$value = $this->getValueFromArray($this->webApplicationConfig, array_reverse(array_merge($path, array($key))));
		}
	}
